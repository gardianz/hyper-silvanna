...
(async () => {
  ...
  // SEGARKAN sesi dulu lewat bot. Cookie Silvana cuma tahan ~1 jam; kalau disuntik
  // dalam keadaan basi browsernya kebuka tapi tetap di halaman login.
  try {
    const M = require(path.join(ROOT, 'index.js'));
    const st = M.makeStates().find(x => x.email === acct.email);
    if (!st) throw new Error('akun gak ada di makeStates');
    await M.buildSwapClients(st);
    console.log('sesi Silvana disegarkan lewat bot');
  } catch (e) { console.log('WARNING gagal nyegerin sesi: ' + ((e && e.message) || e)); }
  ...
  const rec = [];
  // Rekam Silvana DAN backend wallet (walley.cc / supanova) — perintah Canton yg
  // sebenarnya dikirim ke sana, bukan ke Silvana. Tanpa ini alur settle-nya gak
  // kelihatan sama sekali.
  const REC = /app\.silvana\.one|api\.walley\.cc|api\.supanova\.app/;
  ctx.on('request', (req) => {
    const u = req.url();
    if (!REC.test(u)) return;
    const h = req.headers();
    const isAction = !!h['next-action'];
    if (!isAction && !/\/api\/|\/v1\/|\/canton\//.test(u)) return;
    let body = null;
    try { body = req.postData(); } catch (_) { }
    rec.push({ t: Date.now(), method: req.method(), url: u, nextAction: h['next-action'] || null, body: body ? String(body).slice(0, 20000) : null });
  });
  ...
  // Tulis rekaman TERUS-MENERUS, bukan cuma di akhir. Kalau browser ditutup manual
  // (atau proses mati), versi lama kehilangan semuanya karena writeFileSync-nya
  // gak pernah kesampaian.
  const flush = () => { try { fs.writeFileSync(OUT, JSON.stringify(rec, null, 1)); } catch (_) { } };
  const flushTimer = setInterval(flush, 3000);
  browser.on('disconnected', () => { flush(); console.log(`\nbrowser ditutup — ${rec.length} request tersimpan → ${OUT}`); process.exit(0); });
  ...
  await page.waitForTimeout(keepSec * 1000).catch(() => { });
